...
<div class="overflow-y-auto max-h-[100vh] sm:max-h-screen px-2 sm:px-4 flex-wrap bg-blue-100 min-h-screen">
                ...
                <div id="modal-{{ $type }}" class="modal fixed inset-0 bg-gray-900 bg-opacity-50 hidden flex justify-center items-center z-[999]">
                    <div class="modal-content bg-white p-6 rounded-lg max-w-xl w-full">
                        ...
                        @if ($document && $document->document_path)
                            ...
                            @if ($isPdf)
                                <iframe src="{{ $path }}" class="w-full h-80" frameborder="0"></iframe>
    ...
    <script>
        function openModal(type) {
            ...
            document.getElementById('sidebar')?.classList.add('z-0');
        }

        function closeModal(type) {
            document.getElementById('modal-' + type).classList.add('hidden');
            document.getElementById('sidebar')?.classList.remove('z-0');
        }
    </script>
</div>
@endsection
