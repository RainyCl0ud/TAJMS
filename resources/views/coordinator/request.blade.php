        ...
        <tbody class="divide-y divide-gray-200">
            ...
            data-attachment="{{ $request->image_url ?? '' }}">
                ...
                <td class="py-2 px-2 truncate">
                    ...
                    {{ ucfirst($request->status) == 'Approved' ? 'bg-green-100 text-green-600' : 
                    (ucfirst($request->status) == 'Rejected' ? 'bg-red-100 text-red-600' : 
                ...
                <td class="py-2 px-2 text-gray-800 truncate max-w-[80px] sm:max-w-[150px] overflow-hidden text-ellipsis whitespace-nowrap">
                    @php
                        ...
                        $diff = $now->diffForHumans($createdAt, true);
                    @endphp
                    {{ $diff }} ago
                </td>
                ...
                <td class="py-2 px-2 text-center truncate">
                    @if($request->status == 'rejected')
                        <button type="button" class="text-gray-500 hover:text-red-700 font-semibold" onclick="showDeleteModal('{{ route('requests.delete', $request->id) }}', event)">
                            ✖
                        </button>
                    @endif
                </td>
